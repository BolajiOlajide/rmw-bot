...
class UserRepo:
	...
	def find_or_create(value, by='slack_uid', user_data=None):
		...
		if user is None:
			user = UserRepo.new_user(
				slack_uid=user_data['id'], slack_name=user_data['name'], full_name=user_data['real_name'],
	...
	def new_user(slack_uid, slack_name, full_name, phone_number):
		user = User(
			slack_uid=slack_uid, slack_name=slack_name, full_name=full_name, phone_number=phone_number)
		user.save()
		return user
